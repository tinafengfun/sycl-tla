...
def build_parser():
    ...
    parser.add_argument("--mixed-bf16-s8-example-exe", default="./build/examples/02_bmg_gemm_mixed_dtype/02_bmg_gemm_bf16_s8_bf16", help="Weight-only bf16 x int8 mixed-dtype example executable.")
    parser.add_argument("--mixed-f16-s8-example-exe", default="./build/examples/02_bmg_gemm_mixed_dtype/02_bmg_gemm_f16_s8_f16_tensorwise", help="Weight-only fp16 x int8 mixed-dtype example executable.")
    ...
    parser.add_argument("--kernel-catalog-source", choices=["persisted", "generator", "expanded_streamk", "expanded_bmg", "layered_bmg", "layered_bmg_scheduler_expanded", "weight_only_codegen"], default="persisted", help="Catalog source for Phase B candidates. 'expanded_bmg' enables the opt-in BMG Gemm/StreamK/DataParallel/SplitK tile expansion and requires rebuilding the benchmark with the generated build plan. 'layered_bmg' adds regular GEMM legal tile/subgroup/stage enumeration on top of expanded_bmg while preserving the legacy fixed-8x4 scheduler path. 'layered_bmg_scheduler_expanded' keeps the same regular GEMM space but widens BF16 scheduler search across legal subgroup/stage combinations. 'weight_only_codegen' emits compile-time sweep candidates for weight-only mixed-dtype bf16/f16 x int8 kernels via autogenerated sources. 'expanded_streamk' is kept as a compatibility alias.")
    ...
    parser.add_argument("--search-strategy", choices=["manual", "baseline", "expanded_bmg", "layered_exhaustive", "bruteforce_scheduler", "weight_only_codegen"], default="manual", help="High-level search preset. 'manual' preserves explicit catalog/build flags. 'baseline' keeps the legacy persisted baseline search. 'expanded_bmg' keeps the legacy expanded BMG search. 'layered_exhaustive' keeps the legacy layered exhaustive search. 'bruteforce_scheduler' widens BF16 scheduler search across legal subgroup/stage combinations and routes Phase B through scheduler-focused preflight batches. 'weight_only_codegen' switches to autogenerated compile-time sweep candidates for weight-only mixed-dtype bf16/f16 x int8 search.")
